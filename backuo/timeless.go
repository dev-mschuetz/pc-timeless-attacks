...
// reports median wall-clock time over many repetitions to smooth out OS
// noise. The reported numbers are for a single burnIter call, i.e. the
// additional compute cost the server pays when serving /slow vs /fast.
func runCalibrate() {
	iterCounts := []int{10, 30, 100, 300, 1000, 3000, 10000, 30000, 100000}
	const reps = 2001
	fmt.Println("calibrating burnIter on this machine ...")
	fmt.Printf("%-10s %-12s %-12s %-12s\n", "iters", "median_ns", "p10_ns", "p90_ns")
	for _, n := range iterCounts {
		...
		med := samples[reps/2]
		p10 := samples[reps/10]
		p90 := samples[reps-reps/10-1]
		fmt.Printf("%-10d %-12d %-12d %-12d\n", n, med.Nanoseconds(), p10.Nanoseconds(), p90.Nanoseconds())
	}
	fmt.Println()
	fmt.Println("Tip: pick -slow-iter values that span roughly 100ns – 10µs")
	fmt.Println("for a fine-grained sweep. Re-run calibrate if you change")
	fmt.Println("CPU governor / power plan / machine.")
}
